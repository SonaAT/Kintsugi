import streamlit as st
from openai import OpenAI
import os
from gtts import gTTS
import base64
import requests

# App title
flag = 0
st.set_page_config(page_title="🤙💬 Llama 2 Chatbot")

DJANGO_USER_API = "http://127.0.0.1:8000/iris/api/get_user/"
DJANGO_TRIGGER_API = "http://127.0.0.1:8000/iris/api/save_trigger/"

def get_logged_in_user():
    try:
        session_cookies = {"sessionid": st.query_params.get("sessionid", "")}  # Get session ID from URL
        response = requests.get(DJANGO_USER_API, cookies=session_cookies)

        if response.status_code == 200:
            user_data = response.json()
            username = user_data.get("username", "Guest")
            st.session_state.username = username  # Store in session state
            return username
        else:
            return "Guest"
    except Exception as e:
        return "Guest"

st.session_state.username = get_logged_in_user()
st.sidebar.write(f"Logged in as: {st.session_state.username}")  # Debugging output


# LLaMA API Credentials
with st.sidebar:
    st.title('🤙💬 Llama 2 Chatbot')
    st.write('This chatbot is created using the LLaMA 2 LLM model.')

    # Initialize the OpenAI client with LLaMA API
    client = OpenAI(
        api_key= os.environ['LLAMA_API_TOKEN'],
        base_url="https://api.llmapi.com/"  # Replace with your actual LLaMA API base URL
    )

    st.subheader('Models and parameters')
    selected_model = "llama3-8b"  # Default model
    temperature = st.sidebar.slider('temperature', min_value=0.01, max_value=1.0, value=0.1, step=0.01)
    top_p = st.sidebar.slider('top_p', min_value=0.01, max_value=1.0, value=0.9, step=0.01)
    max_length = st.sidebar.slider('max_length', min_value=20, max_value=80, value=50, step=5)
    st.markdown('📖 Learn how to build this app in this [blog](https://blog.streamlit.io/how-to-build-a-llama-2-chatbot/)!')

# Store LLM generated responses
if "messages" not in st.session_state.keys():
    st.session_state.messages = [{"role": "assistant", "content": f"Hi {st.session_state.username}, It's me Kinsuki your personal healing partner. I'm sorry that you are having a panic attack, Let's guide you through this shall we?"}]
if "trigger" not in st.session_state.keys():
    st.session_state.trigger = None  # Initialize trigger as None
if "waiting_for_trigger" not in st.session_state.keys():
    st.session_state.waiting_for_trigger = False 

# Display or clear chat messages
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        st.write(message["content"])

def clear_chat_history():
    st.session_state.messages = [{"role": "assistant", "content": f"Hi {st.session_state.username}, It's me Kinsuki your personal healing partner. I'm sorry that you are having a panic attack, Let's guide you through this shall we?"}]
    st.session_state.trigger = None
    st.session_state.waiting_for_trigger = False

st.sidebar.button('Clear Chat History', on_click=clear_chat_history)

def send_trigger_to_django(trigger):
    username = st.session_state.get("username", "Guest")  # Get username
    st.sidebar.write(f"Sending trigger for: {username}")  # Debugging output

    data = {
        "username": username,  # Use the logged-in user's username
        "trigger": trigger
    }

    try:
        response = requests.post(DJANGO_TRIGGER_API, json=data)
        st.sidebar.write(f"Server Response: {response.status_code} - {response.text}")  # Debugging output

        if response.status_code == 201:
            st.sidebar.success("Trigger saved to database!")
        else:
            st.sidebar.error(f"Failed to save trigger: {response.text}")
    except Exception as e:
        st.sidebar.error(f"Error connecting to Django API: {str(e)}")

# Function for generating LLaMA2 response using OpenAI client
def generate_llama2_response(prompt_input):
    string_dialogue = ("You are a helpful assistant. You only respond once as 'Assistant'. The user is having a panic attack, "
                      "Please guide 'User' through a step-by-step sensory grounding exercise, "
                      "and give each sensory grounding in each step and only move to the next one once 'User' replies. "
                      "Do not ask the 'User' to elaborate the feelings or their thoughts. "
                      "Strictly stick to the sensory grounding exercise. Do not use emojis. "
                      "Please respond in a calm and empathetic tone.")

    for dict_message in st.session_state.messages:
        if dict_message["role"] == "user":
            string_dialogue += "User: " + dict_message["content"] + "\n\n"
        else:
            string_dialogue += "Assistant: " + dict_message["content"] + "\n\n"

    if st.session_state.waiting_for_trigger and prompt_input:
        st.session_state.trigger = prompt_input  # Store the trigger
        st.session_state.waiting_for_trigger = False
        send_trigger_to_django(prompt_input)
        return f"Thank you for sharing that, {st.session_state.username}. I've noted it down. How can I assist you further?"
    
    if prompt_input and "i'm better" in prompt_input.lower():
        st.session_state.waiting_for_trigger = True
        return f"I'm glad to hear that, {st.session_state.username}. What do you think triggered this panic attack?"

    # Use OpenAI client to call LLaMA API
    chat_completion = client.chat.completions.create(
        messages=[
            {"role": "system", "content": string_dialogue},
            {"role": "user", "content": prompt_input or "Start the sensory grounding exercise"}
        ],
        model=selected_model,# Use the selected model (llama2-7b or llama2-13b)
        temperature=temperature,
        top_p=top_p,
        max_tokens=max_length,
        stream=False
    )
    return chat_completion.choices[0].message.content

def generate_audio(text):
    tts = gTTS(text)
    tts.save("response.mp3")
    audio_file = open("response.mp3", "rb").read()
    audio_base64 = base64.b64encode(audio_file).decode("utf-8")
    audio_html = f"<audio autoplay='true'><source src='data:audio/mp3;base64,{audio_base64}' type='audio/mp3'></audio>"
    st.markdown(audio_html, unsafe_allow_html=True)

# Panic button
if st.button("🚨 Panic Button"):
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            response = generate_llama2_response(None)
            placeholder = st.empty()
            full_response = response  # No streaming, direct response
            placeholder.markdown(full_response)
            st.write("Generating audio response...")
            generate_audio(full_response)
    placeholder.markdown("Before you try to concentrate on your surroundings, shall we take a look at your breathing?")
    if flag == 0:
        VIDEO_URL = "https://www.youtube.com/watch?v=BEO8Wze5JbI&list=LL&index=7"
        st.video(VIDEO_URL, format="mp4", start_time=0, subtitles=None, end_time=None, loop=True, autoplay=True, muted=False)
        flag = 1

    message = {"role": "assistant", "content": full_response}
    st.session_state.messages.append(message)

# User input for prompts
if user_input := st.chat_input(placeholder="Type your response here..."):
    st.session_state.messages.append({"role": "user", "content": user_input})
    with st.chat_message("user"):
        st.write(user_input)

    # Generate response to user input
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            response = generate_llama2_response(user_input)
            placeholder = st.empty()
            full_response = response  # No streaming, direct response
            placeholder.markdown(full_response)
            st.write("Generating audio response...")
            generate_audio(full_response)

    message = {"role": "assistant", "content": full_response}
    st.session_state.messages.append(message)
    if st.session_state.trigger:
        with st.sidebar:
            st.write(f"Stored Trigger: {st.session_state.trigger}")